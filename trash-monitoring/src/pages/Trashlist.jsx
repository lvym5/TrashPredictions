...
const getRoundedPercent = (can) => {
  const percent = parseFloat(can.percent_full);
  return isNaN(percent) ? 0 : Math.ceil(percent * 100);
};
    ...
    const fetchData = async () => {
      try {
        const [sensorRes, locRes] = await Promise.all([
          ...
          axios.get("https://vmya5xaguym4xlqfqhethh75wa0hnkrq.lambda-url.us-east-2.on.aws/test")
        ]);

        // Merge sensor and location data by index.
        const merged = sensorRes.data.map((sensor, index) => {
          const loc = locRes.data[index];
          if (!loc) {
            // If no corresponding location, return the sensor data on its own.
            return { ...sensor };
          }
          return { ...loc, ...sensor };
        }).filter((item) => item.sensor_id !== undefined);
            ...
            {sortedData.map((can) => {
              ...
              return (
                <tr key={can.transaction_id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-center">{can.battery_level ?? "N/A"}%</td>
                  <td className="px-4 py-2 text-center">{can.signal_strength ?? "N/A"}</td>
